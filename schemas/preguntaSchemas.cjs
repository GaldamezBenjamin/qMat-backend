...
const opcionesSchema = z.object({
    ...
    correcta: z.enum(['a', 'b', 'c', 'd'], {
        errorMap: () => ({ message: "La opción correcta debe ser 'a', 'b', 'c' o 'd'." })
    }),
}).strict('Campos no permitidos en las opciones.');

// Schema for creating a new question
const createQuestionSchema = z.object({
    enunciado: z.string().min(10, 'El enunciado de la pregunta debe tener al menos 10 caracteres.'),
    dificultad: z.enum(['Baja', 'Media', 'Alta', 'Muy Alta'], {
        errorMap: () => ({ message: "La dificultad debe ser 'Baja', 'Media', 'Alta' o 'Muy Alta'." })
    }),
...
const updateQuestionSchema = z.object({
    enunciado: z.string().min(10, 'El enunciado de la pregunta debe tener al menos 10 caracteres.').optional(),
    dificultad: z.enum(['Baja', 'Media', 'Alta', 'Muy Alta'], {
        errorMap: () => ({ message: "La dificultad debe ser 'Baja', 'Media', 'Alta' o 'Muy Alta'." })
    }).optional(),
    opciones: opcionesSchema.optional(), // Allows updating the entire options object
    id_subcategoria: z.string().min(1, 'El ID de la subcategoría es obligatorio.').optional(),
